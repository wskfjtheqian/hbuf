...
func (b *Builder) printSubscribeCode(dst *build.Writer, data *ast.DataType, tag *ast.Tag) error {
	name := build.StringToHumpName(data.Name.Name)
	dst.Tab(0).Code("func (g ").Code(name).Code(") Subscribe(ctx context.Context, handler func(ctx context.Context, msg *").Code(name).Code(") error) (*nats.Subscription, error) {\n")
...
func (b *Builder) printJetStreamPublishCode(dst *build.Writer, data *ast.DataType, tag *ast.Tag) error {
	name := build.StringToHumpName(data.Name.Name)
	dst.Tab(0).Code("func (g ").Code(name).Code(") JsPublish(ctx context.Context, stream string, options ...hmq.PublishOption) (*jetstream.PubAck, error) {\n")
	dst.Tab(1).Code("return hmq.JetStreamPublish(ctx, stream, \"").Code(name).Code("\", &g, options...)\n")
	dst.Tab(0).Code("}\n\n")
	return nil
}
