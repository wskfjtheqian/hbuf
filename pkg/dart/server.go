...
func printServerRouter(dst io.Writer, typ *ast.ServerType) {
	_, _ = dst.Write([]byte("class " + toClassName(typ.Name.Name) + "Router extends ServerRouter"))
	...
	_, _ = dst.Write([]byte("  " + toClassName(typ.Name.Name) + "Router(this.server){\n"))
	...
	_ = build.EnumMethod(typ, func(method *ast.FuncType, server *ast.ServerType) error {
		...
		_, _ = dst.Write([]byte("     	   return await server." + toFieldName(method.Name.Name) + "(data as "))
		printType(dst, method.Param.Type(), false)
