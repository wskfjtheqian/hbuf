	...
	err := build.EnumField(data, func(field *ast.Field, data *ast.DataType) error {
		...
		for i, val := range verify.GetFormat() {
			...
			if build.IsEnum(field.Type) {
				...
				switch t {
				case build.Int8:
					b.verifyNum(dst, pName, val, f, "-?[1-9]\\\\d*", field.Type, "–128", "127")
				case build.Int16:
					b.verifyNum(dst, pName, val, f, "-?[1-9]\\\\d*", field.Type, "-32768", "32767")
				case build.Int32:
					b.verifyNum(dst, pName, val, f, "-?[1-9]\\\\d*", field.Type, "-2147483648", "2147483647")
				case build.Uint8:
					b.verifyNum(dst, pName, val, f, "[1-9]\\\\d*", field.Type, "0", "255")
				case build.Uint16:
					b.verifyNum(dst, pName, val, f, "[1-9]\\\\d*", field.Type, "0", "65535")
				case build.Uint32:
					b.verifyNum(dst, pName, val, f, "[1-9]\\\\d*", field.Type, "0", "4294967295")
				case build.Float, build.Double:
					b.verifyNum(dst, pName, val, f, "-?[1-9]\\\\d*.\\\\d*|0.\\\\d*[1-9]\\\\d*", field.Type, "", "")
				case build.Int64:
					b.verifyNum(dst, pName, val, f, "[1-9]\\\\d*", field.Type, "-9223372036854775808", "9223372036854775808")
				case build.Uint64:
					b.verifyNum(dst, pName, val, f, "[1-9]\\\\d*", field.Type, "0", "18446744073709551615615")
				case build.Date:
				...
				case build.Decimal:
					...
					if i == len(verify.GetFormat())-1 {
						dst.Code("\tif (!new RegExp(\"-?[1-9]\\\\d*.\\\\d*|0.\\\\d*[1-9]\\\\d*\").test(value")
